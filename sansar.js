...
setInterval(function () {
  if (db.get("huntbot") != "kapalı") {
    sansar.channels.cache.get(huntkanal).send("owo h");
  }
}, 20000);
setInterval(function () {
  if (db.get("huntbot") != "kapalı") {
    sansar.channels.cache.get(huntkanal).send("owo battle");
  }
}, 25000);
setInterval(function () {
  if (db.get("huntbot") != "kapalı") {
    sansar.channels.cache.get(huntkanal).send("owo sell all");
  }
}, 30000);
setInterval(function () {
  if (db.get("huntbot") != "kapalı") {
    sansar.channels.cache.get(huntkanal).send("owo pray");
  }
}, 300000);

sansar.on("message", (message) => {
...
sansar.on("message", (message) => {
  if (message.channel.id == huntkanal || message.channel.type == "dm") {
    if (message.author.id == "408785106942164992") {
      if (
        message.content.includes("Beep Boop") ||
        message.content.includes("Please DM me") ||
        message.content.includes("human") ||
        message.content.includes("check")
      ) {
        db.set("huntbot", "kapalı");
        sansar.channels.cache.get(huntkanal).send("captha geldi @everyone");
      }
    }
  }
});
